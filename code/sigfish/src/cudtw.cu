...
__device__ void init_aln2(int i, int32_t *cuda_aln_rid, int32_t *cuda_aln_pos_st, int32_t *cuda_aln_pos_end, 
    ...
    for (int l = 0; l < SECONDARY_CAP; l++){
        cuda_aln_rid[i * 485 + l] = rid;
        cuda_aln_pos_st[i * 485 + l] = pos;
        cuda_aln_pos_end[i * 485 + l] = pos;
        cuda_aln_score[i * 485 + l] = score;
        cuda_aln_score2[i * 485 + l] = score2;
        cuda_aln_d[i * 485 + l] = d;
        cuda_aln_mapq[i * 485 + l] = 0;
                            ...
                            int32_t qlen, int32_t rlen)
{
    ...
    for (; l < SECONDARY_CAP; l++){
        if (score > cuda_aln_score[i * 485 + l]){
            break;
        }
        else{
            continue;
    ...
    if (l != 0){
        ...
        //     // aln[m] = aln[m + 1];
        // }
        cuda_aln_score[i * 485 + l - 1] = score;
        cuda_aln_pos_end[i * 485 + l - 1] = pos;
        cuda_aln_rid[i * 485 + l - 1] = rid;
        cuda_aln_d[i * 485 + l - 1] = d;

        cuda_aln_pos_st[i * 485 + l - 1] = pos - qlen + 1;
    }
}
                            ...
                            char *cuda_aln_d, uint8_t *cuda_aln_mapq, int32_t *cuda_indexs, int32_t *cuda_i_indexes)
{
    ...
    if (cuda_len_raw_signal[i] > 0 && cuda_et_n[i] > 0){   
        ...
        for (int j = 0; j < SIZE_NUM_REF; j++){
            ...
            if(i>0 && i<485){
                dtw_subsequence((cuda_query + cuda_indexs[i]), &cuda_core_ref_f[j], qlen, rlen, (cuda_cost + (max_qlen * max_rlen * i)));
            }
        ...
        // cuda_db_aln_score[i] = cuda_aln_score[i * 485 + SECONDARY_CAP - 1];
        // cuda_db_aln_score2[i] = cuda_aln_score[i * 485 + SECONDARY_CAP - 2];
        // cuda_db_aln_pos_st[i] = cuda_aln_d[i * 485 + SECONDARY_CAP - 1] == '+' ? cuda_aln_pos_st[i * 485 + SECONDARY_CAP - 1] : cuda_rlen[cuda_aln_rid[i * 485 + SECONDARY_CAP - 1]] - cuda_aln_pos_end[i * 485 + SECONDARY_CAP - 1];
        // cuda_db_aln_pos_end[i] = cuda_aln_d[i * 485 + SECONDARY_CAP - 1] == '+' ? cuda_aln_pos_end[i * 485 + SECONDARY_CAP - 1] : cuda_rlen[cuda_aln_rid[i * 485 + SECONDARY_CAP - 1]] - cuda_aln_pos_st[i * 485 + SECONDARY_CAP - 1];

        // cuda_db_aln_pos_st[i] += cuda_ref_st_offset[cuda_aln_rid[i * 485 + SECONDARY_CAP - 1]];
        // cuda_db_aln_pos_end[i] += cuda_ref_st_offset[cuda_aln_rid[i * 485 + SECONDARY_CAP - 1]];
        // cuda_db_aln_rid[i] = cuda_aln_rid[i * 485 + SECONDARY_CAP - 1];
        // cuda_db_aln_d[i] = cuda_aln_d[i * 485 + SECONDARY_CAP - 1];
...
void dtw_cuda_db(core_t *core, db_t *db){
    ...
    for (int i = 0; i < SIZE_NUM_REF; i++)
    {
        ...
        }// Why this gives segmentation fault?????? fixed
    ...
    for (int i = 0; i < SIZE; i++)
    {
        ...
        for (int j = 0; j < qlen[i]; j++)
        {
            ...
            {
                query[index-1 + qlen[i] - 1 - j] = db->et[i].event[j + qstart[i]].mean;
            }
            else
            {
    ...
    cudaMemcpy(db_aln_maqq, cuda_db_aln_mapq, sizeof(uint8_t) * SIZE, cudaMemcpyDeviceToHost);
    CUDA_CHK();
    cudaMemcpy(i_indexes, cuda_i_indexes, sizeof(int32_t) * (SIZE+10), cudaMemcpyDeviceToHost);
    CUDA_CHK();

    for(int i=0; i<(SIZE+10); i++){
        fprintf(stderr,"%d ", i_indexes[i]);
    }
